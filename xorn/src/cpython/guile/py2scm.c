/* Copyright (C) 2013-2020 Roland Lutz
   ...
   static void py2scm_exception(void)
   {
	   ...
	   if (pvalue_str != NULL && PyObject_IsTrue(pvalue_str)) {
		   error_message = PyUnicode_AsUTF8(pvalue_str);
	   }
   
	   scm_throw(scm_from_utf8_symbol("python-exception"),
		   scm_list_2(scm_from_locale_string(
				   ((PyTypeObject *)ptype)->tp_name),
			   error_message ? scm_from_locale_string(error_message) : SCM_BOOL_F));
	   /* does not return */
   ...
   SCM py2scm(PyObject *value)
   {
	   ...
	   if (PyUnicode_Check(value)) {
		   ...
		   if (s == NULL)
			   return NULL;
		   return scm_from_utf8_stringn(s, size);
	   }

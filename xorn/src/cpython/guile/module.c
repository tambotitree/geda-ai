/* Copyright (C) 2013-2020 Roland Lutz
   ...
   static PyObject *load(PyObject *self, PyObject *args, PyObject *kwds)
   {
	   ...
	   if (!PyUnicode_Check(name_arg)) {
		   char buf[BUFSIZ];
		   snprintf(buf, BUFSIZ, "load() argument 1 must be str, not %s",
   ...
   {
	   const char *s = PyUnicode_AsUTF8(string_arg);
	   SCM string = scm_from_utf8_stringn(s, PyUnicode_GET_LENGTH(string_arg));
   ...
   static PyObject *eval_string(PyObject *self, PyObject *args, PyObject *kwds)
   {
	   ...
	   if (!PyArg_ParseTupleAndKeywords(
			   args, kwds, "U:eval_string", kwlist, &string_arg))
		   return NULL;
   ...
   static struct PyModuleDef guilemodule = {
   ...
   PyMODINIT_FUNC PyInit_guile(void)
   {
	   if (PyType_Ready(&ProcedureType) < 0)
		   return NULL;
   
	   PyObject *module = PyModule_Create(&guilemodule);
	   if (!module)
		   return NULL;
   
	   Py_INCREF(&ProcedureType);
	   if (PyModule_AddObject(module, "Procedure", (PyObject *)&ProcedureType) < 0)
		   return NULL;
	   ...
	   guile_error = PyErr_NewExceptionWithDoc(
		   ...
		   NULL, NULL);
	   if (guile_error == NULL ||
		   PyModule_AddObject(module, "GuileError", guile_error) < 0)
		   return NULL;
